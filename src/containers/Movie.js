...
const Movie = ({
  ...
  // Fetch movie id when id in the url changes
  useEffect(() => {
    scroll.scrollToTop({
      smooth: true,
      delay: 500,
    ...
    fetchRecommended(match.params.id, params.page);
        ...
        <MovieWrapper>
          ...
          <ImageWrapper style={!loaded ? { display: 'none' } : {}}>
            <MovieImg
              ...
              onError={e => {
                setError(true);
                if (e.target.src !== `${NotFoundSvg}`) {
                  e.target.src = `${NotFoundSvg}`;
                }
              }}
            />
          </ImageWrapper>
          <MovieDetails>
            ...
            <DetailsWrapper>
              <RatingsWrapper>
                <Rating ratingDividedByTwo={movie.vote_average / 2} />
            ...
            <Text>
              {movie.overview ? movie.overview : 'Synopsis unavailable...'}
            </Text>
...
const mapStateToProps = ({ movie, config, recommended }) => ({
  movie,
  config,
  recommended,
});
